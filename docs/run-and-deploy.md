...
浏览器页面可以沿用同源 OpenResty 代理；Tauri 桌面版没有网页来源可继承，因此首次连接服务前，点击左下角“运行模式”右侧的设置图标，填写后端的完整地址，例如：
...
点击“保存并连接”后，地址与运行模式会保存在当前设备。更换地址会清除原服务的登录态，避免把令牌发送到另一台服务。服务端默认已允许 Tauri WebView 的来源；若你通过 `FORGEDESK_ALLOWED_ORIGINS` 覆盖了默认配置，需要同时保留 `http://tauri.localhost`、`https://tauri.localhost` 和 `tauri://localhost`。
