...
import app.forgedesk.interfaces.rest.TraceContext;
...
public class HttpTranslationProviderGateway implements TranslationGateway {
  ...
  @Override
  public String translate(TranslationCommand request, TranslationCredentials credentials) {
    TranslationProvider provider = request.provider();
  ...
  private String baidu(TranslationCommand request, TranslationCredentials credentials) {
  ...
  private String youdao(TranslationCommand request, TranslationCredentials credentials) {
  ...
  private String google(TranslationCommand request, TranslationCredentials credentials) {
    ...
    try {
      String payload =
          objectMapper.writeValueAsString(
              Map.of(
                  ...
                  request.text(),
                  "source",
                  googleLanguage(request.sourceLanguage()),
                  "target",
                  googleLanguage(request.targetLanguage()),
                  "format",
                  "text"));
          ...
          HttpRequest.newBuilder(
                  URI.create(
                      "https://translation.googleapis.com/language/translate/v2?key="
                          + percentEncode(credentials.appKey())))
              .timeout(REQUEST_TIMEOUT)
              .header("Content-Type", "application/json")
              .header(TraceContext.HEADER, TraceContext.currentId())
      ...
      return body.path("data")
          ...
          .path(0)
          .path("translatedText")
          .asText(errorMessage(body, "Google 翻译未返回结果"));
    } catch (Exception exception) {
      throw wrap(exception);
    }
  }

  private String alibaba(TranslationCommand request, TranslationCredentials credentials) {
    ...
    try {
      ...
      if (response.statusCode() >= 400) {
        throw new TranslationException(
            errorMessage(body, "翻译服务请求失败（HTTP " + response.statusCode() + "）"));
      }
      return body;
  ...
  private String errorMessage(JsonNode body, String fallback) {
    return body.path("error_msg")
        .asText(
            body.path("Error")
                .path("Message")
                .asText(body.path("error").path("message").asText(fallback)));
  }

  private void require(String value, String label) {
    if (value == null || value.isBlank()) {
      throw new TranslationException("请填写" + label);
    }
  }
  ...
  private String truncate(String value) {
    return value.length() <= 20
        ? value
        : value.substring(0, 10) + value.length() + value.substring(value.length() - 10);
  }

  private String baiduLanguage(String language) {
    return language.equals("zh") ? "zh" : language;
  }

  private String youdaoLanguage(String language) {
    return language.equals("zh") ? "zh-CHS" : language;
  }

  private String googleLanguage(String language) {
    return language.equals("zh") ? "zh-CN" : language;
  }

  private String alibabaLanguage(String language) {
    return language.equals("zh") ? "zh" : language;
  }
}
